...
for im=1:length(meshtypes)
    ...
    pathname = fullfile(getfemobjectoptions('path'),'MYCODE',...
        'results','FCBA',filename,[elemtype meshtype]);
...
if solveProblem
    ...
    switch lower(meshtype)
        ...
        case 'unstructured'
            ...
            switch lower(loading)
                case 'uniform'
                    S = build_model(Q,'cl',cl,'elemtype',elemtype,'filename',fullfile(pathname,['gmsh_plate_rect_' elemtype]));
                case 'concentrated'
                    S = build_model(Q,'cl',cl,'elemtype',elemtype,'filename',fullfile(pathname,['gmsh_plate_rect_' elemtype]),'points',x_load);
            end
    end
    ...
    %% Save variables
    save(fullfile(pathname,'problem.mat'),'S','Q','a','b','h','f');
    ...
    load(fullfile(pathname,'problem.mat'),'S','Q','a','b','h','f');
...
%     mysaveas(pathname,'err_Uz',formats,renderer);
...
%     mysaveas(pathname,'err_Rx',formats,renderer);
...
%     mysaveas(pathname,'err_Ry',formats,renderer);
end
