...
for cg=1:length(tolcgs)
    ...
    for k=2:length(myboxes)+1
        ...
        wprec{k} =  zeros(getnbddlfree(Spartf{k}),1);
        w1prec{k} =  zeros(getnbddl(Spartfd{k}),1);
    end
    %
    ...
    for i=1:30
        ...
        for k=2:length(myboxes)+1
            ...
            Bpatchr{k} = freevector(Spartfd{k},bpatch{k}) - freevector(Spartfd{k},Apatch{k}*(Ppatchfgammaf{k}'*Pgammaoutgammaf{k}*Pgamma{k}*U)) - Apatchr{k}*freevector(Spartfd{k},w1prec{k});
            %
            % w1{k} = solve(Apatchr{k},Bpatchr{k});
            dw1{k} = cgs(Apatchr{k},Bpatchr{k},tolcgs(cg));
            dw1{k} = unfreevector(Spartfd{k},dw1{k});
            w1{k}= dw1{k}+unfreevector(Spartfd{k},w1prec{k});
            % w1{k} = spectral_decomposition(w1{k});
            %
...
% end
%%
% save('C:\PROGS\test\erreur_4patchalpha_beta_rho_geoconf_meshnoconf_alpha10_beta0_rho1.mat','errref','Uzref','Uref','wref','rho','PC','S','Spart','w','U','lambda','P','kappa')
%%
figure(451)
[Ur,r] = random(U);
...
maxe = max([max(var_expect_cond(P{1}*U)) max(var_expect_cond(w{2})) max(var_expect_cond(w{3})) max(var_expect_cond(w{4})) max(var_expect_cond(w{5}))])
