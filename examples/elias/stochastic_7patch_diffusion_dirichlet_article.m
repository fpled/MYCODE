...
[X,PC] = PCTPMODEL(RANDVARS(RVUNIFORM(0,1),nbva),'order',[3*ones(1,5),2,3*ones(1,5),2,3,2,2],...
    'typebase',1,'groups',{1:5,6,7:11,12,13,14,15},'fedim',[6,12,14,15],'femesh',{rrr,rrr,rrr2,rrr2},'pcg');
...
% colorbar
%%
lmax = calc_lmax_patch_dirichlet_diffusion(S,Spartf,Spartfd,AS,Atilde,PC,Apatchsep,A2patchsep,psitp,Ppatchfgammaf,Pgammaoutgammaf,Pgamma,Dlfsep,Dlf,numpatchdiff,numpatchdir)
...
for k=2:length(myboxes)+1
    ...
    w1prec{k} = SEPMATRIX([{zeros(getnbddl(Spartfd{k}),1)} getphi(one(PC))],1);
end
...
for ii=1:length(numpatchdir)
    k = numpatchdir(ii)+1
    % wr = randomeval(psitp{k},r).*unfreevector(Spartfd{k},randomeval(PCTPMATRIX(w1{k},PC),r))+ ...
    %  Ppatchfgammaf{k}'*Pgammaoutgammaf{k}*Pgamma{k}*Ur;
    plot_sol(lssplitelem(LSMODEL(Spartf{k},LEVELSET(randomeval(-psitp{k},r)))),randomeval(w{k},r),'edgecolor','none')
end
colorbar
%%
% load('diffusion_dirichlet_multipatch_article_ro02_pgd150_bis')
for k=2:length(myboxes)+1
    % wr = randomeval(psitp{k},r).*unfreevector(Spartfd{k},randomeval(PCTPMATRIX(w1{k},PC),r))+ ...
    %  Ppatchfgammaf{k}'*Pgammaoutgammaf{k}*Pgamma{k}*Ur;
    plot_sol(Spartf{k},randomeval(w{k},r),'edgecolor','none')
...
clf
r=0.4*ones(1,15);
Ur = randomeval(PCTPMATRIX(U,PC),r);
plot_sol(Spart{1},P{1}*Ur,'edgecolor','none')
hold on
...
for ii=1:length(numpatchdiff)
    ...
    k = numpatchdiff(ii)+1
    % w1r = randomeval(PCTPMATRIX(w1{k},PC),r);
    % figure(451+k)
    wr = randomeval(PCTPMATRIX(w1{k},PC),r)+ ...
        Ppatchfgammaf{k}'*Pgammaoutgammaf{k}*Pgamma{k}*Ur;
    % wr = randomeval((w{k}),r);
    plot_sol(Spartf{k},wr,'edgecolor','none')
    plot(surface{k})
    % myprint(chemin,['plot_sol_w_diff_dir_article' num2str(k-1)] ,{'jpeg','epsc2'})
end
% colorbar
for ii=1:length(numpatchdir)
    k = numpatchdir(ii)+1
    % w1r = randomeval(unfreevector(Spartfd{k},PCTPMATRIX(w1{k},PC)),r);
    % figure(451+k+100)
    wr = randomeval(psitp{k},r).*unfreevector(Spartfd{k},randomeval(PCTPMATRIX(w1{k},PC),r))+ ...
        Ppatchfgammaf{k}'*Pgammaoutgammaf{k}*Pgamma{k}*Ur;
    
    % wr = randomeval((w{k}),r);
    plot_sol(lssplitelem(LSMODEL(Spartf{k},LEVELSET(randomeval(-psitp{k},r)))),wr,'edgecolor','none')
    plot(surface{k})
    % myprint(chemin,['plot_sol_w_diff_dir_article' num2str(k-1)] ,{'jpeg','epsc2'})
end
colorbar
% myprint(chemin,'plot_sol_ufinal_diff_dir_article',{'jpeg','epsc2'})
...
% myprint(chemin,['plot_sol_Uglobal_diff_dir_article'] ,{'jpeg','epsc2'})
%%
figure(48)
clf
err = [];for k=1:length(erreur_table)
    err(k) = erreur_table{k}{1};
end
semilogy(err,'Color',rand(1,3),'linewidth',3)
set(gca,'fontsize',16)
xlabel('Iterations')
set(gca,'fontsize',16)
ylabel('Error')
...
for k=2:length(myboxes)+1
    % wr = randomeval(psitp{k},r).*unfreevector(Spartfd{k},randomeval(PCTPMATRIX(w1{k},PC),r))+ ...
    %  Ppatchfgammaf{k}'*Pgammaoutgammaf{k}*Pgamma{k}*Ur;
    w1r = unfreevector(Spartfd{k},randomeval(PCTPMATRIX(w1{k},PC),r));
    % wr = randomeval((w{k}),r);
    plot_sol(Spartf{k},w1r,'edgecolor','none')
end
colorbar
