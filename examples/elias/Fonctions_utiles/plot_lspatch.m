function plot_patch(S,Spart,U,w,Ufinal,Uref,PC,lssep,P,numpatchls,varargin)
%function plot_patch(S,Spart,U,w,Ufinal,Uref,PC,lssep,P,numpatchls,varargin)
%POUR les LSMODEL
% ('m','n') m et n sont les param�tres de subplot
% m*n est le nombre de r�alisations
%C PA BON CETTE FONCTION
...
for nb=1:(m*n)
    ...
    if (length(numpatchls)==2)
        lsr = intersect(LEVELSET(unfreevector(S,P{numpatchls(1)+1}'*randomeval(PCTPMATRIX(lssep{numpatchls(1)+1},PC),r))),LEVELSET(unfreevector(S,P{numpatchls(2)+1}'*randomeval(PCTPMATRIX(lssep{numpatchls(2)+1},PC),r))))
        Sr  = LSMODEL(setmaterial(S,FOUR_ISOT()),lsr);
        Sr = final(Sr);
        Sr = addcl(Sr,[],'u');
    end
