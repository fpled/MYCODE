function [u_exp_a,u_exp_b,coord_a,coord_b,cl_a,cl_b] = extractCorreliJunctionScrew(Job_a,Job_b,Mesh_a,Mesh_b,U_a,U_b,h)
% function [u_exp_a,u_exp_b,coord_a,coord_b,cl_a,cl_b] = extractCorreliJunctionScrew(Job_a,Job_b,Mesh_a,Mesh_b,U_a,U_b,h)
...
% scaleFactorx_a = 3*h/(max(Coordx_a)-min(Coordx_a));
% scaleFactory_a = h/(max(Coordy_a)-min(Coordy_a));
% scaleFactorx_b = h/(max(Coordx_b)-min(Coordx_b));
% scaleFactory_b = 2*h/(max(Coordy_b)-min(Coordy_b));
% scaleFactor = mean([scaleFactorx_a scaleFactory_a scaleFactorx_b scaleFactory_b]);
scaleFactor_a = h/(max(Coordy_a)-min(Coordy_a));
scaleFactor_b = h/(max(Coordx_b)-min(Coordx_b));
...
coordx_b = coordx_b-mean_coordx_a;
coordy_b = coordy_b-mean_coordy_b;
coordy_a = coordy_a-mean_coordy_b;
