...
PFmodel = 'Isotropic'; % values : 'Isotropic', 'Anisotropicmiehe' 'Anisotropicamor'
randMat = 1; % random material parameters. Values : 0, 1.
randPF = 0; % random phase field parameters. Values : 0, 1.
...
if setProblem
    ...
    elseif Dim==3
        ...
        %
        clD = 2e-5; % test
        clC = 1e-5; % test (with nt = 250, crack goes a little bit upward)
        %
        clD = 1e-5; % test
        clC = 1e-5; % test
    end
    S_phase = gmshdomainwithedgecrack(D,C,clD,clC,fullfile(pathname,'gmsh_domain_single_edge_crack'));% ,2,'recombine');
    S = S_phase;
...
if randMat % random material parameters
    ...
    delta1 = 0.1; % <sqrt(5/6). Coefficient of variation for bulk modulus
    la = 1 - 1/delta1^2; % la < 1/5. Parameter controlling the level of statistical fluctuation
    delta2 = 1/sqrt(5/delta1^2 - 4); % < 1/sqrt(2). Coefficient of variation for shear modulus
    %
    mC2 = mu; % mean shear modulus
    switch Dim
        case 3
            mC1 = lambda + 2*mu/3; % mean bulk modulus
            la1 = (1-la)/mC1; % la1 > 0
        ...
        case 2
            mC1 = lambda + mu; % mean bulk modulus
            la1 = (1-la)/mC1; % la1 > 0
            la2 = (1-5*la)/mC2; % la2 > 0
            %
            a1 = 1-la; % a1 > 0
            ...
            C_sample(:,1) = gamrnd(a1,b1,N,1); % [Pa]
            C_sample(:,2) = gamrnd(a2,b2,N,1); % [Pa]
            % lambda_sample = C_sample(:,1) - C_sample(:,2); % [Pa]
            E_sample = (4*C_sample(:,1).*C_sample(:,2))./(C_sample(:,1)+C_sample(:,2)); % [Pa]
            NU_sample = (C_sample(:,1)-C_sample(:,2))./(C_sample(:,1)+C_sample(:,2));
        otherwise
            error('Spatial dimension error')
    end
else
    E_sample = E*ones(N,1);
...
if randPF % random phase field parameters
    delta3 = .1; % coefficient of variation of the fracture toughness
    delta4 = .1; % coefficient of variation of the regularization length
    a3 = 1/delta3^2;
    b3 = gc/a3;
    a4 = 1/delta4^2;
    b4 = l/a4;
...
if solveProblem
    ...
    parfor ii=1:N
        gci = gc_sample(ii);
        li = l_sample(ii);
        mat_phasei = mat_phase;
        % mat_phase = FOUR_ISOT('k',gc*l,'r',gc/l+2*H);
        mat_phasei = setparam(mat_phasei,'k',gci*li);
        mat_phasei = setparam(mat_phasei,'r',gci/li+2*H);
        S_phasei = S_phase;
        S_phasei = setmaterial(S_phasei,mat_phasei);
        %
        Ei = E_sample(ii);
        NUi = NU_sample(ii);
        mati = mat;
        ...
        [~,~,~,ft] = solvePFDetLinElasSingleEdgeCrack(S,S_phase,T,gc,l,loading,L,e,'display',-1);
        f_max(ii) = max(ft);
    end
    Ht = 0;
    dt = 0;
    ...
    ft = 0;
    time = toc(tTotal);
    save(fullfile(pathname,'solution.mat'),'gc_sample','l_sample','E_sample','NU_sample','Ht','dt','ut','ft','f_max','time');
else
    load(fullfile(pathname,'solution.mat'),'gc_sample','l_sample','E_sample','NU_sample','Ht','dt','ut','ft','f_max','time');
end
%
%% Outputs
fprintf('\n');
