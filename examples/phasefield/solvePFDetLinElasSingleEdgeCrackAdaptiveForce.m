function [ft,dmaxt] = solvePFDetLinElasSingleEdgeCrackAdaptiveForce(S_phase,S,T,PFsolver,C,BU,BL,BRight,BLeft,BFront,BBack,loading,initialCrack,sizemap,varargin)
% function [ft,dmaxt] = solvePFDetLinElasSingleEdgeCrackAdaptiveForce(S_phase,S,T,PFsolver,C,BU,BL,BRight,BLeft,BFront,BBack,loading,initialCrack,sizemap,varargin)
...
if display_
    ...
    fprintf('\n+-----------+---------+-----------+-----------+-----------+----------+----------+');
    fprintf('\n| %4d/%4d | %7d | %9.3e | %9.3e | %9.3e | %8d | %8d |\n',0,length(T),0,0,0,0,getnbnode(S),getnbelem(S));
end
...
for i=1:length(T)
    ...
    if display_
        fprintf('| %4d/%4d | %7d | %9.3e | %9.3e | %9.3e | %8d | %8d |\n',i,length(T),nbIter,t(i)*1e3,f*((Dim==2)*1e-6+(Dim==3)*1e-3),dmax,getnbnode(S),getnbelem(S));
    end
