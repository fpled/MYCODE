function [dt,ut,ft,T,St_phase,St,Ht,Edt,Eut,output] = solvePFDetLinElasThreePointBendingAdaptiveThreshold(S_phase,S,T,PFsolver,C,BU,BL,BR,P0,LU,initialCrack,sizemap,varargin)
% function [dt,ut,ft,T,St_phase,St,Ht,Edt,Eut,output] = solvePFDetLinElasThreePointBendingAdaptiveThreshold(S_phase,S,T,PFsolver,C,BU,BL,BR,P0,LU,initialCrack,sizemap,varargin)
...
filename     = getcharin('filename',varargin,'gmsh_three_point_bending_single_edge_crack');
...
if displaySol && displayMesh && displayForce
    posd = [spos(1) fpos(2:4)];
    posm = fpos;
    posf = [spos(3)-fpos(3) fpos(2:4)];
elseif displaySol && displayMesh
...
while ti < tf-eps
    ...
    % else
        ...
        while (errConv > tolConv) && (nbIter < maxIter)
            ...
            if nbIter==1
                ...
                ti = ti + dti;
                ud = ti;
                S = addbcThreePointBending(S,ud,BU,BL,BR,P0);
            end
    ...
    if ti < tf-eps && ~any(db > dbth)
        % Mesh adaptation
        S_phase_old = S_phase;
        S_phase_ref = addbcdamageThreePointBendingAdaptive(S_phase_old,C,BU,BL,BR);
