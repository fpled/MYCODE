...
if setProblem
    ...
    switch lower(meshAdapt)
        ...
        case 'mmg'
            S_phase = adaptmesh(S_phase,cl,fullfile(pathname,'gmsh_Lshaped_panel'),'gmshoptions',gmshoptions,'mmgoptions',mmgoptions_init);
        otherwise
...
if displaySolution
    ...
    if loadExp && strcmpi(loading,'monotonic')
        ...
        xlabel('Displacement [mm]','Interpreter',interpreter)
        ylabel('Force [kN]','Interpreter',interpreter)
        legend('numerical','experimental','Interpreter',interpreter)
