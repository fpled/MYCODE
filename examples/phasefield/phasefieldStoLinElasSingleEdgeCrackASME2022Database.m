...
if setProblem
    ...
    % clC = 5e-6; % [Hu, Guilleminot, Dolbow, 2020, CMAME], [Li, Zhang, Zhou, 2024, CG]
    ...
    switch lower(symmetry)
        case {'isot','meanisot'} % almost surely or mean isotropic material
            switch lower(loading)
                case 'tension'
                    % [Miehe, Hofacker, Welschinger, 2010, CMAME], [Ambati, Gerasimov, De Lorenzis, 2015, CM], [Molnar, Gravouil, 2017, FEAD], [Li, Zhang, Zhou, 2024, CG]
                ...
                case 'shear'
                    ...
                    % du = 1e-5 mm during 2000 time steps (up to u = 23.9e-3 mm)
                    % dt = 1e-8;
                    % nt = 2390;
                    % t = linspace(dt,nt*dt,nt);
...
if displaySolution
    ...
    clf
    scatter(gc_sample*1e-3,fmax*((Dim==2)*1e-6+(Dim==3)*1),'Marker','.','MarkerEdgeColor','r','MarkerFaceColor','r')
    grid on
    box on
    set(gca,'FontSize',fontsize)
    ...
    elseif Dim==3
        ylabel('$f_m$ [N]','Interpreter',interpreter)
    end
    ...
    clf
    scatter(gc_sample*1e-3,fc*((Dim==2)*1e-6+(Dim==3)*1),'Marker','.','MarkerEdgeColor','b','MarkerFaceColor','b')
    grid on
    box on
    set(gca,'FontSize',fontsize)
    xlabel('$g_c$ [N/mm]','Interpreter',interpreter)
    ...
    elseif Dim==3
        ylabel('$f_c$ [N]','Interpreter',interpreter)
    end
    ...
    elseif Dim==3
        ylabel(...%'$f_c$, $f_m$ [N]'
            'Force [N]','Interpreter',interpreter)
    end
