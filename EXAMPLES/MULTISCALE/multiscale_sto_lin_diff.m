%% Multiscale stochastic linear diffusion %%
%%----------------------------------------%%
...
filename = ['multiscale_sto_lin_diff_' num2str(n) '_patches'];
...
for k=1:n
    ...
    % K_patch{k} = ones(getnbnode(patch.S),1,PC) + beta_patch * double(squeeze(f(P))) * X{k};
    % 
    % beta_in = 0;
    % P = POINT(glob.S.node);
