%% Multiscale stochastic linear diffusion circular inclusions anisotropic %%
%%------------------------------------------------------------------------%%
...
if exist([pathname 'gmsh_multi_circular_inclusions.msh'],'file')
    glob.S = gmsh2femobject(2,[pathname 'gmsh_multi_circular_inclusions.msh'],2);
elseif exist([pathname 'gmsh_multi_circular_inclusions.geo'],'file')
    glob.S = gmsh2femobject(2,[pathname 'gmsh_multi_circular_inclusions.geo'],2);
else
    ...
    cl = 0.02;
    glob.S = gmshdomainwithinclusion(D,B,cl,cl,[pathname 'gmsh_multi_circular_inclusions']);
end
...
for k=1:n
    % D_patch{k} = B{k};
    patches.PATCH{k}.S = keepgroupelem(glob.S,2*k);
