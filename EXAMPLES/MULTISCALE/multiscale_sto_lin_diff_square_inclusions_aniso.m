%% Multiscale stochastic linear diffusion square inclusions anisotropic %%
%%----------------------------------------------------------------------%%
...
filename = ['multiscale_sto_lin_diff_' num2str(n) '_square_inclusions_aniso'];
...
for k=1:n
    ...
    K_patch{k} = ones(patch.S.nbnode,1,PC) + double(squeeze(f(patch.S.node))) * g(k) * X{k};
    K_in{k} = 1;
end
...
nbsamples = 100;
if calc_MC_error_estimate_refs
