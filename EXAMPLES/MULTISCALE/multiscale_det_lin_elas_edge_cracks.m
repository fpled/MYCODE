...
cl_patch_P = 0.05;
for k=1:n
    patches.PATCH{k}.S = gmshdomainwithedgecrack(D_patch{k},P_patch{k},cl_patch_D,cl_patch_P,[pathname 'gmsh_patch_' num2str(k) '_edge_crack']);
end
...
% Patches
for k=1:n
    patches.PATCH{k}.S = final(patches.PATCH{k}.S,'duplicate');
end
...
for i=1:2
    ...
    mysaveas(pathname,['U_' num2str(i)],{'fig','epsc2'},renderer);
    
    plot_sol(glob,patches,interfaces,U,w,'displ',i);
    mysaveas(pathname,['sol_' num2str(i)],{'fig','epsc2'},renderer);
