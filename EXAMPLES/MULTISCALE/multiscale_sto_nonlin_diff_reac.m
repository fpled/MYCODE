%% Multiscale stochastic nonlinear diffusion reaction %%
%%----------------------------------------------------%%
...
filename = ['multiscale_sto_nonlin_diff_reac_' num2str(n) '_patches'];
...
rv = RVUNIFORM(0,1);
RV = RANDVARS(repmat({rv},1,2*n));
[X,PC] = PCMODEL(RV,'order',1,'pcg','typebase',1);
X_K = cellfun(@(patch) X{2*patch.number-1},patches.PATCH,'UniformOutput',false);
...
method = METHOD('type','leastsquares','display',true,'displayiter',false,...
    ...
    'sampling','adaptive','initsample',2,'addsample',0.1,'maxsample',Inf,...
    'regul','','cv','leaveout','k',10,...
    'tol',1e-3,'tolstagn',1e-1,'toloverfit',1.1,'correction',false,...
    'decompKL',false,'tolKL',1e-12,'cvKL','leaveout','kKL',10);
...
for k=1:n
    ...
    K_patch{k} = ones(getnbnode(patch.S),1,PC) + double(squeeze(f(P))) * X_K{patch.number};
    R_patch{k} = double(squeeze(f(P))) * X_R{patch.number};
    % K2_patch{k} = double(squeeze(f(P))) * X_K2{patch.number};
