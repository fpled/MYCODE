...
for k=2:length(myboxes)+1
    Apatch{k} = PCRADIALMATRIX([getnbddl(Spartf{k}),getnbddl(Spartf{k})],PCphi{k});
    Lk = getL(km{k});
...
for k=2:length(myboxes)+1
    bpatch{k} = PCRADIALMATRIX([getnbddlfree(Spartf{k}),1],PCphi{k});
    bpatch{k} = bpatch{k}+lk{Spartf{k}}(:)*one(PCphi{k});
end
...
elseif (length(myboxes)==4)
    ...
    if couplage==1
        ...
        Aref = [A11{1},sp1{2},sp1{3},sp1{4},sp1{5},project(A13{2},PC),project(A13{3},PC),A13{4},A13{5};...
            sp1{2}',project(A22{2},PC),sp2{3},sp2{4},sp2{5},A23{2},spg2{3},spg2{4},spg2{5};...
...
for cg=1:length(tolcgs)
    ...
    for i=1:30
        ...
        for k=2:length(myboxes)+1
            ...
            elseif couplage==2 %grossier
                temp = temp - project(Pgamma{k}'*Dl{k}*lambdaprec{k},PC);
            end
            
            if i~=1
                temp = temp +  Atilde{k}*calc_masse(Uprec,PC)*one(PC);
            elseif i==1
                temp = temp +  Atilde{k}*Uprec*one(PC);
            end
        end
        ...
        for k=2:length(myboxes)+1
            ...
            Bpatchr{k} = freevector(Spartfd{k},bpatch{k}) - freevector(Spartfd{k},Apatch{k}*(Ppatchfgammaf{k}'*Pgammaoutgammaf{k}*Pgamma{k}*project(U,PCphi{k})));
            %
            w1{k} = solve(Apatchr{k},Bpatchr{k});
            % w1{k} = cgs(Apatchr{k},Bpatchr{k},tolcgs(cg));
            w1{k} = unfreevector(Spartfd{k},w1{k});
            %
            w{k} =  w1{k} + Ppatchfgammaf{k}'*Pgammaoutgammaf{k}*Pgamma{k}*project(U,PCphi{k});
            %
            % blambda = Ppatchfgammaf{k}*Apatch{k}*w{k}-Ppatchfgammaf{k}*calc_vector(LINFORM(0),Spartf{k});
            blambda= project(Ppatchfgammaf{k}*Apatch{k}*Ppatchfgammaf{k}'*Pgammaoutgammaf{k}*Pgamma{k}*project(U,PCphi{k}),PC)+(Ppatchfgammaf{k}*project((Apatch{k}'*unfreevector(Spartfd{k},w1{k})),PC)-Ppatchfgammaf{k}*calc_vector(LINFORM(0),Spartf{k}));
            %
            if couplage==1
...
hold on
for k=2:length(myboxes)+1
    plot_sol(Spartf{k},randomeval(w{k},r(k-1)),'edgecolor','none')
end
colorbar
%%
