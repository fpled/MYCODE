function repA = rho_optimal(V,S,Spartfd,AS,Atilde,Apatch,Ppatchfgammaf,Pgammaoutgammaf,Pgamma,Dlf)
% function repA = rho_optimal(V,S,Spartfd,AS,Atilde,Apatch,Ppatchfgammaf,Pgammaoutgammaf,Pgamma,Dlf)

n = getnbddlfree(S);
tempphi = zeros(1,n);
for k=2:length(Apatch)
    Bpatchr{k} = - freevector(Spartfd{k},Apatch{k}*(Ppatchfgammaf{k}'*Pgammaoutgammaf{k}*Pgamma{k}*V));
    Apatchr{k} = freematrix(Spartfd{k},Apatch{k});
    %
    w1{k} = solve(Apatchr{k},Bpatchr{k});
    w1{k} = unfreevector(Spartfd{k},w1{k});
    %
    w{k} = w1{k} + Ppatchfgammaf{k}'*Pgammaoutgammaf{k}*Pgamma{k}*V;
    %
    % blambda = Ppatchfgammaf{k}*Apatch{k}*w{k} - Ppatchfgammaf{k}*calc_vector(LINFORM(0),Spartf{k});
    blambda = Ppatchfgammaf{k}*Apatch{k}*Ppatchfgammaf{k}'*Pgammaoutgammaf{k}*Pgamma{k}*V + (Ppatchfgammaf{k}*(Apatch{k}'*unfreevector(Spartfd{k},w1{k})));
    %
